/* u8x8_d_bitmap.c a bitmap device */
...
uint8_t u8x8_bitmap_SetSize(u8x8_bitmap_t* b, uint16_t pixel_width,
  uint16_t pixel_height)
{
  if(b->u8x8_buf != NULL)
    free(b->u8x8_buf);
  ...
  b->u8x8_buf = (uint8_t*)malloc(
    (size_t)b->tile_width * (size_t)b->tile_height * (size_t)8 * (size_t)2);
  b->u8g2_buf = b->u8x8_buf
    + (size_t)b->tile_width * (size_t)b->tile_height * (size_t)8;

//  printf("$$$ %d %d %lu %p %p\n", b->tile_width, b->tile_height, n, &b->u8x8_buf[0], &b->u8x8_buf[n - 1]);
  if(b->u8x8_buf == NULL) {
    b->u8g2_buf = NULL;
    return 0;
  }
...
void u8x8_bitmap_DrawTiles(u8x8_bitmap_t* b, uint16_t tx, uint16_t ty,
...
uint8_t* u8x8_get_buffer(void)
{
  return u8x8_bitmap.u8x8_buf;
}
...
//  ( {auto t = x; x = y; y = t;});
  ...
  x = 199 - x;
//  y = 167 - y;
  uint32_t pos = (x / 2) * 17 + y / 4;
  if(pos >= 5100)
    return;
  int x3 = 1 - (x % 2);
  int y3 = 3 - (y % 4);
  auto& targ = vram[(x / 2) * 51 + y / 4];
  targ = (!c) ? (targ | (1 << (y3 * 2 + x3))) : (targ & ~(1 << (y3 * 2 + x3)));
  //  if(c) vram[pos] |= (1 << (y3 * 2 + x3)); else vram[pos] &= ~(1 << (y3 * 2 + x3));
}

void u8x8_bitmap_transfer(
  void (*write_vram_f)(int x1, int y1, int x2, int y2, const void* data))
{
  u8x8_bitmap_t* b = &u8x8_bitmap;
  uint16_t x, y;

  for(y = 0; y < b->pixel_height; y++) {
    for(x = 0; x < b->pixel_width; x++) {
      pset(x, y, (u8x8_bitmap_GetPixel(b, x, y) == 0));
    }
  }
  write_vram_f(0, 0, 199, 199, vram);
}

/*========================================================*/

static uint8_t u8x8_d_bitmap(u8x8_t* u8x8, uint8_t msg, uint8_t arg_int,
  void* arg_ptr)
{
  u8g2_uint_t x, y, c;
  uint8_t* ptr;
...
void u8g2_SetupBitmap(u8g2_t* u8g2, const u8g2_cb_t* u8g2_cb,
  uint16_t pixel_width, uint16_t pixel_height)
{
  ...
  u8g2_SetupBuffer(u8g2, u8x8_bitmap.u8g2_buf, (pixel_height + 7) / 8,
    u8g2_ll_hvline_vertical_top_lsb, u8g2_cb);
}

/*========================================================*/

static uint8_t u8x8_d_bitmap_chain(u8x8_t* u8x8, uint8_t msg, uint8_t arg_int,
  void* arg_ptr)
{
...
{
  if(u8x8_SetBitmapDeviceSize(u8x8, u8x8_GetCols(u8x8) * 8,
    u8x8_GetRows(u8x8) * 8) == 0)
    return 0;

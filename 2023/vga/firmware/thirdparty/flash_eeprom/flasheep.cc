...
void FlashEEP::write_word(unsigned short dummy_addr, unsigned short data)
{
...
unsigned short FlashEEP::read_word(unsigned short dummy_addr)
{
    int addr = get_first_empty_addr();
    if(addr == 0 || addr == -1)
...
void FlashEEP::write(unsigned short addr, void* data, int nbytes)
{
...
void FlashEEP::read(unsigned short addr, void* data, int nbytes)
{
    unsigned short* pdata = (unsigned short*)data;
    addr &= (~1);
    if(nbytes % 2 != 0)
...
unsigned short FlashEEP::get_size(void)
{
    return EEPROM_SIZE;
}

FlashEEP::FlashEEP(size_t base_addr, size_t page_size)
{
    _base = base_addr;

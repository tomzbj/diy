...
enum {
    CMD_DISP_OFF = 0xae,
    CMD_DISP_ON = 0xaf,
    CMD_STARTLINE_0 = 0x40,
    CMD_SET_CONTRAST = 0x81,
    CMD_SEG_REMAP_COL127_SEG0 = 0xa1,
    CMD_COM_SCAN_DIR_REMAPPED = 0xc8,
    CMD_DISP_NORMAL = 0xa6,
    CMD_DISP_INVERSE = 0xa7,
    CMD_MULTIPLEX_RATIO = 0xa8,
    CMD_DISP_OFFSET = 0xd3,
    CMD_DISP_CLK_DIVR_FREQ = 0xd5,
    CMD_PRECHARGE_PERIOD = 0xd9,
    CMD_COM_PINS_HW_CONF = 0xda,
    CMD_SET_VCOMH_DESELECT_LEVEL = 0xdb,
    CMD_SET_MEM_ADDR_MODE = 0x20,
    CMD_SET_CHARGE_PUMP_ENABLE_1 = 0x8d,
    CMD_SET_CHARGE_PUMP_ENABLE_2 = 0x14,
    CMD_SET_ENTIRE_DISP_ON = 0xa4
};
...
{
    g.write_reg_f(&g, 0x00, cmd);
}

static void WriteData(unsigned char* data, int size)
{
    g.write_regs_f(&g, 0x40, data, size);
}
...
void SSD1306_Init(zi2c_t* i2c)
{
    ...
    const unsigned char cmd_list[] = {
        //
        0xae, 0x00, 0x10, 0x40, 0xb0, 0x81, 0xcf, 0xa1, 0xa6, 0xa8, 0x3f, 0xc0,
        0xa0, 0xd3, 0x00, 0xd5, 0xf0, 0xd9, 0xf1, 0xda, 0x02, 0xdb, 0x49, 0x8d,
        0x14, 0x20, 0x02, 0xaf    //
        };

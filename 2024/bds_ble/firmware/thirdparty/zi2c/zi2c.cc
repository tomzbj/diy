...
//    stop();
    ...
    uint8_t val = readbyte_nack();
    stop();
    return val;
}

void ZI2C::write_regs(uint8_t reg, const uint8_t* data, int count) const
{
    start();
    writebyte_waitack(_addr << 1);
    writebyte_waitack(reg);
...
void ZI2C::read_raw(uint8_t reg, uint8_t* data, int count) const
{
    start();
    writebyte_waitack((_addr << 1) + 1);
//    writebyte_waitack(reg);
...
void ZI2C::read_regs(uint8_t reg, uint8_t* data, int count) const
{
    start();
    writebyte_waitack(_addr << 1);
    writebyte_waitack(reg);

...
class ZCli {
  public:
    ...
    int bind(const char* name, void (*cmd_f)(void))
    {
      return bind(name, "", (cmd_func_t)cmd_f);
    }
    ...
    int bind(const char* name, const char* cmd_type, void (*cmd_f)(void))
    {
      return bind(name, cmd_type, (cmd_func_t)cmd_f);
    }
    ...
    void parse(const void* pmsg, int size)
    {
      ...
      while(token != NULL) {
        ...
        token = strsep(&p, seps);    // Get next token:
      }
      if(count == 0)
        return;

...
class ZPin {
  ...
  private:
    ...
    // No constexpr/const on this table: CMSIS GPIOA is (Type*)BASE, and
    // integer→pointer is not a C++ constant expression. A const static object
    // still needs a constant-expression initializer, so keep it non-const.
    // (const here would mean "pointer const", not "GPIO regs read-only".)
    static inline ZPIN_PORT_TYPE gpio_port[] = {
